...
test_that("encoding an example produces expected results", {
  input <- "The quick brown fox jumps over the lazy dog. 1!2@3#4$5%6^7&8*9(0)-_=+`~|[{]};:/?.>,<"
  output <- base64_encode(charToRaw(input))
  # the following encoding is based on: https://www.base64encode.net/
  expected_output <-
    "VGhlIHF1aWNrIGJyb3duIGZveCBqdW1wcyBvdmVyIHRoZSBsYXp5IGRvZy4gMSEyQDMjNCQ1JTZeNyY4KjkoMCktXz0rYH58W3tdfTs6Lz8uPiw8"
...
test_that("decoding encoded strings produces original string", {
  ...
  original <- "The quick brown fox jumps over the lazy dog. 1!2@3#4$5%6^7&8*9(0)-_=+`~|[{]};:/?.>,<"
  encoded <- base64_encode(charToRaw(original))
  decoded <- base64_decode(encoded)
  expect_equal(original, rawToChar(decoded))
})
...
test_that("padding characters are appended as expected", {
  # encoded strings should be 4/3 the length of the original string, however, the encoded strings
  # must have lengths divisible by 4.
