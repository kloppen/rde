...
test_that("line widths are as expected", {
  ...
  sapply(encoded_lines[-length(encoded_lines)], function(l) {  # the last line may be shorter
...
test_that("copied data.frame with no line breaks is encoded and decoded correctly", {
  encoded <- copy_rde_var(iris, line.width = -1, no.clipboard = TRUE)
  expect_type(encoded, "character")
  expect_false(grepl("\\n", encoded))

  decoded <- load_rde_var(useCache = TRUE, loadFcn = {}, encoded)
...
test_vector <- function(vec, add.linebreaks) {
  encoded <- copy_rde_var(vec, line.width = if (add.linebreaks) 80L else -1L,
                           no.clipboard = TRUE)
  expect_type(encoded, "character")
  expect_equal(grepl("\\n", encoded), add.linebreaks)
  decoded <- load_rde_var(useCache = TRUE, loadFcn = {}, encoded)
  expect_identical(vec, decoded)
}

test_that("copied atomic vector (character) is encoded and decoded correctly", {
  test_vector("test vector", FALSE)
})

test_that("copied atomic vector (integer) is encoded and decoded correctly", {
  test_vector(44L, FALSE)
})

test_that("copied atomic vector (float) is encoded and decoded correctly", {
  test_vector(1.1, FALSE)
})

test_that("copied atomic vector (boolean) is encoded and decoded correctly", {
  test_vector(TRUE, FALSE)
})

test_that("copied matrix (of floats) is encoded and decoded correctly", {
