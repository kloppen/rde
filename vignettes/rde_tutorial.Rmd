...
Let's take an example. Let's say that we have a spreadsheet of populations of the ten most populus countries (data originally taken from [@popclock]). Somewhere near the top of our R Notebook, we have a code chunk that looks like the following:
...
If we run that code as is, it will raise a warning. We would expect this since there is nothing in the `cache` argument, so of course, the result of the `loadFcn` and `cache` are different. We'll need to fill in cache argument of `load_rde_var`.
